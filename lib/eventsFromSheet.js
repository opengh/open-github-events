...
function parseDate (date, timeZone) {
	...
	var parts = /^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})\s+([0-9]{1,2})\:([0-9]{1,2})\:([0-9]{1,2})$/.exec(date)
	var year
	var month
	var date
	var hour
	...
	if (parts) {
		...
		parts = /^([0-9]{4})\/([0-9]{1,2})\/([0-9]{1,2})\s+([0-9]{1,2})\:([0-9]{1,2})\:([0-9]{1,2})$/.exec(date)
		if (parts) {
			year = parts[1]
			month = parts[2]
			date = parts[3]
			...
			parts = /^([0-9]{1,2})\/([0-9]{1,2})\/([0-9]{4})$/.exec(date)
			year = parts[3]
			month = parts[1]
			date = parts[2]
			hour = '00'
